...
const Products = () => {
  ...
  const performAPICall = async () => {
    let answer = await axios.get(`${config.endpoint}/products`)
    .catch((e) => {
      if (e.response) {
        //enqueueSnackbar(e.response.data.message, { variant: "error" });
        console.log(e.response.data.message);
      } else {
        console.log("Something went wrong. Check the backend console for more details");
        // enqueueSnackbar(
  ...
  const debounceSearch = (event, debounceTimeout) => {
    //console.log("time out",debounceTimeout);
